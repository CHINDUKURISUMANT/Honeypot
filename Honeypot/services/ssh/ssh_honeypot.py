...
print(f"[SSH] Starting honeypot — paramiko {paramiko.__version__}", flush=True)
...
def load_or_generate_host_key(path: str) -> paramiko.RSAKey:
    ...
    if os.path.exists(path):
        print(f"[SSH] Loading existing host key from {path}", flush=True)
        return paramiko.RSAKey(filename=path)

    print(f"[SSH] Generating new host key and saving to {path}", flush=True)
...
try:
    ...
    print(f"[SSH] FATAL: Could not load/generate host key: {_key_err}", flush=True)
    sys.exit(1)
...
class HoneypotSSHServer(paramiko.ServerInterface):
    ...
    def __init__(self, client_ip: str):
        ...
        self.shell_request = threading.Event()
...
class FakeShell:
    ...
    # Strict boundary between deterministic and AI layers.
    # CORE/CONTENT/SYSTEM → NEVER call AI, always local handler.
    # ATTACK              → AI override allowed (CONTAIN path).
    # UNKNOWN_CMD         → always "command not found", never AI.
    # ─────────────────────────────────────────────
    _CORE_CMDS = {
    ...
    _ATTACK_TOOLS = {
        ...
        "msfconsole", "metasploit", "sqlmap", "nikto", "dirb",
        "dirbuster", "gobuster", "ffuf", "wfuzz", "exploit",
        "payload", "reverse", "pty", "xterm", "socat",
    }
    _META_CHARS = ("|", "&&", ";", "$(", "<<", ">>", "`", "||")

    @classmethod
    def _classify_cmd(cls, base: str, cmd: str) -> str:
        """Classify a command into CORE/CONTENT/SYSTEM/ATTACK/UNKNOWN_CMD."""
        has_metachar = any(c in cmd for c in cls._META_CHARS)
        if has_metachar:
            return "ATTACK"
        if base in cls._CORE_CMDS:
            return "CORE"
        if base in cls._CONTENT_CMDS:
            return "CONTENT"
        if base in cls._SYSTEM_CMDS:
            return "SYSTEM"
        if base in cls._ATTACK_TOOLS:
            return "ATTACK"
        return "UNKNOWN_CMD"
    ...
    def __init__(self, username: str, client_ip: str, channel):
        ...
        # UID is derived once from the username hash — NOT from the RNG sequence.
        # Previously _rng.randint() was called for UID, but since _rng advances
        # with every ls -l call the UID value shifted each session, breaking
        # consistency between `whoami` and `id`.
        self._uid = 0 if username == "root" else ((hash(username) & 0xFFFF) % 4) + 1000
    ...
    def handle(self, cmd: str) -> bool:
        ...
        base = parts_raw[0] if parts_raw else ""

        # ─────────────────────────────────────────────
        # STRICT COMMAND CLASSIFICATION
        # Determines AI routing BEFORE any event is sent.
        # CORE/CONTENT/SYSTEM → always fire-and-forget, local handler runs.
        # ATTACK              → synchronous + AI override allowed.
        # UNKNOWN_CMD         → always "command not found", no AI.
        # ─────────────────────────────────────────────
        cmd_type = self._classify_cmd(base, cmd)
        ...
        # ─── UNKNOWN_CMD: always produce "command not found", no AI ───
        ...
        # ─── CORE / CONTENT / SYSTEM: fire-and-forget, no AI block ───
        api_response = None
        if cmd_type in ("CORE", "CONTENT", "SYSTEM"):
            threading.Thread(
                target=send_event,
                args=("SSH_COMMAND", event_payload),
                daemon=True,
            ).start()
            # Fall through to deterministic handlers below.

        # ─── ATTACK: synchronous + allow CONTAIN AI override ───
            ...
            if (
                ...
                output = api_response["terminal_output"].strip()
                for line in output.split("\n"):
                    self._out(line)
                return True
            # If no AI output, fall through to best-effort local handler.
        ...
        if base in ("exit", "logout", "quit"):
            ...
            # Use _uid computed once at session init (hash-stable, not RNG-position-dependent)
            self._out(f"uid={self._uid}({self.username}) gid={self._uid}({self.username}) groups={self._uid}({self.username}),4(adm),27(sudo)")
        ...
        elif base in ("chmod", "chown", "touch", "mkdir", "rm", "cp", "mv"):
            pass  # Silent success
        ...
        elif cmd_type == "ATTACK" and not api_response:
            # Compound commands that got no AI response: produce realistic fallback
            # Split on the first metachar and show result for first part only
            first_cmd = cmd.split(";")[0].split("|")[0].split("&&")[0].strip()
            first_base = first_cmd.split()[0] if first_cmd.split() else base
            if first_base in self._CORE_CMDS | self._SYSTEM_CMDS | self._CONTENT_CMDS:
                # Re-dispatch the first part deterministically
                self.handle(first_cmd)
            else:
                self._out(f"bash: {base}: command not found")
...
def handle_client(client, addr):
    ...
    try:
        ...
        if server.exec_command is not None:
            try:
                cmd = server.exec_command.decode("utf-8", errors="ignore")
            except Exception:
                cmd = str(server.exec_command)
            shell.handle(cmd)
